/**
 * npm-pkgs-cli <https://github.com/tunnckoCore/npm-pkgs-cli>
 *
...
test('npm-pkgs-cli:', function() {
  ...
  test('should throw TypeError when `callback` is not a function [npm-pkgs-filter]', function(done) {
    function fixture() {
      npmPkgs('tunnckocore', 'jstransformer-*', {one: 'two'});
    }
  ...
  test('should work properly with filters when `patterns` given', function(done) {
    npmPkgs('tunnckocore', 'jstransformer-*', function _cb(err, res) {
      test.ifError(err);
      test.is.array(res);
      test.is.number(res.length);
  ...
  test('should error when non existing user and filter patterns given', function(done) {
    npmPkgs('fjk43hkjhhhhhhhhhhhhhhhkjgg3k4g234', ['*js*', 'npm-*'], function _cb(err, res) {
      test.is.error(err);
      test.is.number(err.code);
      test.is.undefined(res);
      test.equal(err instanceof Error, true);
